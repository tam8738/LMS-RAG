...
import { documentPreviewPath } from "../routes";
...
export const teacherDocumentService = {
  ...
  /**
   * Open the standalone viewer. PDF.js requests only the byte ranges it needs.
   */
  async previewDocumentFile(documentId: number): Promise<void> {
    const previewTab = window.open(documentPreviewPath(documentId), "_blank");
    if (!previewTab) {
